-- Say which quarter turnover is measured against.
--
-- "Prior" has meant the most recent filing before this one, which is the
-- previous quarter only when every quarter in between has a filing. Norges
-- Bank's Q1 2026 filing is a confidential-treatment placeholder and is now
-- stored inactive, so the filing before Q2 2026 is Q4 2025 - and six months of
-- change was about to be reported as a quarter's. Every archetype threshold is
-- calibrated on quarterly turnover, so that inflates it and can flip a
-- classification.
--
-- The function cannot decide what to do about a gap; that belongs with the
-- classifier, which already refuses a measurement whose basis is unsound and
-- says why in the caveats. What it can do is stop hiding the gap. It now
-- returns the date it compared against, and the caller can see that Q2 2026
-- was measured against Q4 2025 rather than assuming otherwise.
--
...
as $$
  ...
  prev as (select r.id, r.manager_id, r.report_date from ranked r where r.rk = 2),
  book as (
    select c.manager_id,
           h.cusip,
           h.value_usd,
  ...
  left join prev pv on pv.manager_id = a.manager_id
...
comment on function public.institutional_strategy_metrics(uuid) is
  'Per-manager 13F book shape: breadth, concentration, turnover, persistence, voting and 13D/13G counts. prior_report_date says which quarter turnover is measured against - it is not always the previous one. Holding period is per name; everything else is per reported line. Pass a manager id; null measures every manager and is only safe from a direct connection.';
