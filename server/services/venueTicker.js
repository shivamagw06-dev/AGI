 ...
 *
 * The recovery is a guess, so it is made to prove itself. Stripping a
 * currency suffix is easy and produces a plausible ticker almost every time;
 * what makes it safe is refusing the result unless the issuer we hold and
 * the company the SEC lists under that ticker are the same company. Without
 * that check, a symbol whose tail merely resembles a currency code hands its
 * position to whatever unrelated ticker the trimmed string happens to spell.
 */
...
export function candidateBases(symbol) {
  ...
  return [...new Set(out)].filter((base) => /^[A-Z]{1,5}$/.test(base));
}

/**
 * Recover a ticker, or explain why not.
 *
 * `secByTicker` maps a US ticker to its normalised company name; `issuerName`
 * is the normalised name on the holding. Both come from the caller so this
 * stays free of network and database.
 */
export function recoverTicker(symbol, issuerName, secByTicker, { normalise }) {
  const bases = candidateBases(symbol);
  if (!bases.length) return { ticker: null, reason: 'no currency suffix' };

  const held = normalise(issuerName || '');
  ...
  for (const base of bases) {
    ...
    if (secName === held) matched.push(base);
  }
  ...
  if (!matched.length) {
    ...
    return {
      ticker: null,
      reason: known.length
        ? `trimmed to ${known.join('/')} but the issuer is "${held}", not "${known.map((b) => secByTicker.get(b)).join('/')}"`
        : `trimmed to ${bases.join('/')}, none of which the SEC lists`,
    };
  }
