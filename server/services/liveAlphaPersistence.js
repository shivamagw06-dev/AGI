...
export class LiveAlphaPersistence {
  ...
  async saveAlphaRun(result, diagnostics = {}) {
    ...
    const rows = result.signals.map((signal) => ({
      ...
      liquidity_ok: signal.liquidity_ok, factor_values: { ...signal.factors, liquidity_verified: signal.liquidity_verified ?? null, liquidity_reason: signal.liquidity_reason ?? null, residual_15m: signal.residual_15m, residual_60m: signal.residual_60m, volume_surprise: signal.volume_surprise, sector_strength: signal.sector_strength, sector_instrument_key: signal.sector_instrument_key ?? null },
