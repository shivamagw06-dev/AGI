...
Supabase both time out). Production runs use the Render scheduler instead:

  GROWW_EQUITY_OPPORTUNITY_SCHEDULER=true  (finance-news-backend on Render)

See server/services/growwEquityOpportunityRun.js

Groww Cloud vault — set these env vars (never hardcode secrets in this file):
  GROWW_TOTP_TOKEN, GROWW_TOTP_SECRET
  RESEARCH_SIGNALS_INGEST_TOKEN, RESEARCH_SIGNALS_INGEST_SECRET  (optional; delivery blocked from Groww Cloud)
"""
import hashlib
import hmac
import json
...
DEFAULT_UNIVERSE = (
    "RELIANCE,HDFCBANK,ICICIBANK,INFY,TCS,BHARTIARTL,ITC,LT,SBIN,AXISBANK,KOTAKBANK,"
    "HINDUNILVR,BAJFINANCE,MARUTI,SUNPHARMA,NTPC,TITAN,ULTRACEMCO,ASIANPAINT,POWERGRID,"
    "M&M,NESTLEIND,TATASTEEL,ONGC,TECHM,WIPRO,COALINDIA,JSWSTEEL,HCLTECH,ADANIENT,ADANIPORTS,"
    "BAJAJFINSV,GRASIM,DRREDDY,CIPLA,EICHERMOT,HEROMOTOCO,APOLLOHOSP,BRITANNIA,DIVISLAB,"
    "INDUSINDBK,TATACONSUM,SBILIFE,HDFCLIFE,BPCL,SHRIRAMFIN,TRENT,BEL,JIOFIN,BAJAJ-AUTO"
)
...
def return_pct(values, n):
    if len(values) <= n or not values[-n - 1]:
        return None
...
def main():
    ...
    limit = max(5, min(200, int(os.getenv("AGI_MAX_SYMBOLS", "50"))))
    symbols = [
        symbol.strip().upper()
    ...
    candidates = [{**row, "rank": index + 1, "signal": "research_candidate"} for index, row in enumerate(rows[:10])]
    deteriorating = [{**row, "signal": "risk_review"} for row in sorted(rows, key=lambda item: item["score"])[:10]]
