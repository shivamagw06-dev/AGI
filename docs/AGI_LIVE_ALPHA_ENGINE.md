...
Strategy #5, derivatives positioning, classifies long buildup, short buildup, short covering and long unwinding from synchronized 15-minute futures price and open-interest changes. Each universe member may provide a current-expiry `derivativeInstrumentKey`; those keys are added to the existing feed subscription. The strategy fails closed unless at least 10 mapped contracts have valid current and lagged OI. The example universe intentionally contains no expiring contract identifiers, so production must refresh and verify these mappings before the engine can run.
