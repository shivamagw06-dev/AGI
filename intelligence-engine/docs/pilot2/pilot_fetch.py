    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            if attempt == MAX_RETRIES - 1:
                state["failures"].append({"url": url[:120], "kind": "HTTPError",
                                          "code": exc.code, "attempts": attempt + 1})
                raise
            state["retries"] += 1
            state["retry_detail"].append({"url": url[:120], "kind": "HTTPError",
                                          "code": exc.code, "attempt": attempt + 1,
                                          "backoff_s": BACKOFF_BASE * (2 ** attempt)})
            time.sleep(BACKOFF_BASE * (2 ** attempt))
        except (urllib.error.URLError, TimeoutError, OSError) as exc:
            if attempt == MAX_RETRIES - 1:
                state["failures"].append({"url": url[:120], "kind": type(exc).__name__,
                                          "detail": str(exc)[:120], "attempts": attempt + 1})
                raise
            state["retries"] += 1
            state["retry_detail"].append({"url": url[:120], "kind": type(exc).__name__,
                                          "detail": str(exc)[:120], "attempt": attempt + 1,
                                          "backoff_s": BACKOFF_BASE * (2 ** attempt)})
            time.sleep(BACKOFF_BASE * (2 ** attempt))
