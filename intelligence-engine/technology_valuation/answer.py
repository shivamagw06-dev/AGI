...
def format_technology_answer(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if (result.get("model") or {}).get("subsector") in {"ERD_TECHNOLOGY_SERVICES","HARDWARE_ELECTRONICS","DATA_CENTRES","FINTECH_PAYMENTS","CYBERSECURITY_CLOUD"}:
        return _format_specialized_answer(result)
...
def _format_specialized_answer(result:dict[str,Any])->dict[str,Any]:
    company=result.get("company_id") or "The company";model=result["model"];v=result["valuation"];k=result["kpis"]
    family=model["subsector"]
    if family=="ERD_TECHNOLOGY_SERVICES":focus=f"Book-to-bill is {k['book_to_bill']:.2f}x, utilization {k['utilization']:.1%}, embedded mix {k['embedded_mix']:.1%} and FCF margin {k['fcf_margin']:.1%}."
    elif family=="HARDWARE_ELECTRONICS":focus=f"Unit growth is {k['unit_growth']:.1%}, ASP {k['asp']:.2f}, inventory turns {k['inventory_turns']:.2f}x and FCF margin {k['fcf_margin']:.1%}."
    elif family=="DATA_CENTRES":focus=f"Operational capacity is {k['operational_mw']:.1f} MW, occupancy {k['occupancy']:.1%}, PUE {k['pue']:.2f}x and FCF margin {k['fcf_margin']:.1%}."
    elif family=="FINTECH_PAYMENTS":focus=f"TPV growth is {k['tpv_growth']:.1%}, net take rate {k['take_rate']:.2%}, contribution margin {k['contribution_margin']:.1%} and fraud loss rate {k['fraud_loss_rate']:.3%}."
    else:focus=f"ARR growth is {k['arr_growth']:.1%}, NRR {k['nrr']:.1%}, gross margin {k['gross_margin']:.1%} and RPO coverage {k['rpo_coverage']:.2f}x."
    return {"status":"RESEARCH_ONLY","direct_conclusion":f"{company} trades at {v['current_ev_ebitda']:.2f}x EBITDA and {v['current_ev_sales']:.2f}x revenue. AGI treats the view as scenario-dependent, not execution-ready.","business_model":model["economic_structure"],"operating_view":focus,"valuation":v,"market_implied_expectations":result.get("market_expectations"),"scenarios":result.get("scenarios"),"key_risks":model.get("valuation_risks") or [],"ai_impact":result.get("business_economics",{}).get("ai_analysis"),"what_to_monitor":result.get("monitoring") or [],"confidence":result.get("confidence"),"as_of":result.get("as_of"),"sources":sorted({x.get("source_id") for x in (result.get("provenance") or {}).values() if x.get("source_id")}),"limitations":"Operational specialized-technology research, not investment-certified or personalized advice.","execution_eligible":False}
