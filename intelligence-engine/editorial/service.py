...
def unsupported_numeric_claims(
    ...
    *,
    ...
    """Numbers generated by the writer that have no structured evidence.

    Formatting differences such as ``4,820`` versus ``4820`` are normalised.
    The policy is intentionally conservative: the editorial layer is a writer,
    not a calculator, so it may restate supplied figures but may not create new
    ones. If a transformation is needed later, it must be supplied explicitly
    in the structured package rather than inferred here.
    """
    produced = _numbers_in(text)
    allowed = _numbers_in(structured or {}) | _numbers_in(question or "")
    return sorted(produced - allowed, key=lambda item: (Decimal(item), item))
    ...
    def _run(
        ...
        *,
        ...
        try:
            if is_enabled():
                ...
                unsupported_numbers = unsupported_numeric_claims(
                    text or "", clean, question=question)
        ...
        out = {
            ...
            "provider_output_rejected": bool(unsupported_numbers),
            "numeric_grounding": {
