...
SPECS = (
    ...
    CalculationSpec("GROSS_MARGIN", "Gross Margin", "profitability", "gross profit / revenue", ("gross_profit", "revenue"), "decimal", lambda i: _div(i["gross_profit"], i["revenue"])),
    ...
    CalculationSpec("CONSUMER_PRICE_VOLUME_MIX_GROWTH", "Consumer Price-Volume-Mix Growth", "consumer", "(1 + volume growth) * (1 + price/mix growth) - 1", ("volume_growth","price_mix_growth"), "decimal", lambda i: (1.0+i["volume_growth"])*(1.0+i["price_mix_growth"])-1.0, allow_mixed_units=True, methodology="Deterministic decomposition; acquisitions and currency require separate bridges"),
    CalculationSpec("CONSUMER_SALES_PER_STORE", "Consumer Sales per Store", "consumer_unit_economics", "revenue / average stores", ("revenue","opening_stores","closing_stores"), "currency_per_store", lambda i: _div(i["revenue"],_avg(i,"opening_stores","closing_stores")), allow_mixed_units=True),
    CalculationSpec("CONSUMER_SALES_DENSITY", "Retail Sales Density", "consumer_unit_economics", "revenue / average retail area", ("revenue","opening_retail_area","closing_retail_area"), "currency_per_area", lambda i: _div(i["revenue"],_avg(i,"opening_retail_area","closing_retail_area")), allow_mixed_units=True),
    CalculationSpec("CONSUMER_INVENTORY_TURNS", "Consumer Inventory Turns", "consumer_quality", "cost of goods sold / average inventory", ("cogs","opening_inventory","closing_inventory"), "multiple", lambda i: _div(i["cogs"],_avg(i,"opening_inventory","closing_inventory")), allow_mixed_units=True),
    CalculationSpec("CONSUMER_REVPAR", "Hotel Revenue per Available Room", "hotels", "average daily rate * occupancy", ("adr","occupancy"), "currency_per_room_night", lambda i: i["adr"]*i["occupancy"], allow_mixed_units=True),
    CalculationSpec("CONSUMER_ROOM_REVENUE", "Hotel Room Revenue Bridge", "hotels", "rooms * available days * occupancy * average daily rate", ("rooms","available_days","occupancy","adr"), "currency", lambda i: i["rooms"]*i["available_days"]*i["occupancy"]*i["adr"], allow_mixed_units=True, methodology="Room-revenue bridge; excludes food, events and other revenue"),
    CalculationSpec("CONSUMER_REVENUE_PER_PAIR", "Footwear Revenue per Pair", "footwear", "footwear revenue / pairs sold", ("revenue","pairs_sold"), "currency_per_pair", lambda i: _div(i["revenue"],i["pairs_sold"]), allow_mixed_units=True),
    CalculationSpec("CONSUMER_LEASE_ADJUSTED_LEVERAGE", "Lease-adjusted Net Debt to EBITDA", "consumer_risk", "(net debt + lease liabilities) / EBITDA", ("net_debt","lease_liabilities","ebitda"), "multiple", lambda i: _div(i["net_debt"]+i["lease_liabilities"],i["ebitda"]), allow_mixed_units=True),
    CalculationSpec("CONSUMER_EV_PER_KEY", "Hotel Enterprise Value per Key", "hotel_valuation", "enterprise value / rooms", ("enterprise_value","rooms"), "currency_per_room", lambda i: _div(i["enterprise_value"],i["rooms"]), allow_mixed_units=True),
)
