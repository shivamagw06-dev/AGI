...
_TOOLS = (
    ...
    ToolSpec("SCREEN_RELATIVE_VALUATION", "1.0", "read", "Screen the versioned annual sector-ratio warehouse by peer valuation, own history and quality conditions; outputs are research-only and explicitly not live.", "valuation_intelligence.screen.screen_relative_valuation", {"metric": _field("str", required=True, limit=30), "sector": _field("str", limit=80), "window_years": _field("int", limit=10), "limit": _field("int", limit=100), "sort": _field("str", limit=30), "max_historical_percentile": _field("int", limit=100), "min_discount_pct": _field("int", limit=100), "min_roe": _field("int", limit=100), "min_ebitda_margin": _field("int", limit=100)}, max_calls=3),
...
_FINANCIAL_RE = re.compile(r"\b(revenue|margins?|ebitda|profits?|cash flows?|valuation|multiples?|median|percentile|p/e|p/b|ev/ebitda|roe|roa|roic|nim|npa|loans?|deposits?|financials?|financially|invest|investment|expensive|cheap)\b", re.I)
...
_VALUATION_SCREEN_RE = re.compile(r"\b(which|screen|rank|cheapest|most expensive|stretched|below (?:its|their|the)|discount to|premium to|historical percentile|sector median|peer median|value trap)\b", re.I)
...
def plan_tools(question: str, *, ticker_hint: str | None = None) -> dict[str, Any]:
    ...
    if _VALUATION_SCREEN_RE.search(query) and _FINANCIAL_RE.search(query):
        selected.append("SCREEN_RELATIVE_VALUATION")
        reasons["SCREEN_RELATIVE_VALUATION"] = "cross_sectional_historical_and_peer_valuation_screen"
