...
_IT_SERVICES_RE = re.compile(r"\b(tcs|infosys|infy|hcltech|hcl tech|wipro|tech mahindra|techm|it services|outsourcing|utilization|billing rate|attrition|deal wins|tcv|book.to.bill)\b", re.I)
...
def plan_tools(question: str, *, ticker_hint: str | None = None) -> dict[str, Any]:
    ...
    if ticker_hint and _FINANCIAL_RE.search(query) and not _IT_SERVICES_RE.search(query):
        selected.append("GET_FINANCIAL_VALUATION")
        reasons["GET_FINANCIAL_VALUATION"] = "authoritative_financial_subsector_valuation"
    if ticker_hint and _IT_SERVICES_RE.search(query):
