...
_TOOLS = (
    ...
    ToolSpec("GET_TECHNOLOGY_VALUATION", "2G.0", "read", "Evaluate classified Technology, Telecom and tower-infrastructure companies using point-in-time evidence, deterministic sector economics and execution-blocking gates.", "technology_valuation.service.evaluate_technology_company", {"company": _field("dict", required=True, limit=30), "inputs": _field("dict", required=True, limit=100), "as_of": _field("str", required=True, limit=32), "peers": _field("list", limit=50), "history": _field("list", limit=100), "scenarios": _field("dict", limit=3)}, max_calls=3),
...
def plan_tools(question: str, *, ticker_hint: str | None = None) -> dict[str, Any]:
    ...
    if ticker_hint and _FINANCIAL_RE.search(query) and not _TECHNOLOGY_RE.search(query):
