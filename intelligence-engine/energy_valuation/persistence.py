"""Persist Phase 5 curricula through the shared sector registry."""
from __future__ import annotations
from typing import Any,Callable
from financials_valuation.persistence import persist_sector_certification,seed_sector_model
from energy_valuation.models import MODELS,PARENT_SECTOR
Transport=Callable[...,Any]
def seed_energy_models(*,transport:Transport)->dict[str,Any]:
 rows=[seed_sector_model(model,parent_sector=PARENT_SECTOR,transport=transport) for model in MODELS.values()]
 return {"ok":all(x.get("ok") for x in rows),"models":len(rows),"results":rows,"investment_certified_models":0}
def persist_energy_certification(result:dict[str,Any],*,transport:Transport)->dict[str,Any]:
