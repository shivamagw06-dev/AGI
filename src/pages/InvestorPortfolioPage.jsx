import { useInvestorDisclosures } from '@/components/institutions/ExchangeDisclosures';
import { unifiedHoldings } from '@/lib/unifiedHoldings';
import priceMappings from '@/data/investorProfiles/priceMappings.json';
...
export default function InvestorPortfolioPage() {
  ...
  const disclosures = useInvestorDisclosures(investorId, country === 'IN');
  const holdings = useMemo(() => unifiedHoldings(currentProfile, disclosures.profile?.rows || [], priceMappings.IN), [currentProfile, disclosures.profile]);
  ...
  const rows = useMemo(() => holdings.filter(row =>
    `${row.stock} ${row.holder || ''} ${row.security || ''}`.toLowerCase().includes(query.trim().toLowerCase()) &&
    (status === 'all' || disclosureStatus(row) === status)), [holdings, query, status]);
  const statuses = [...new Set(holdings.map(disclosureStatus))];
  ...
  const download = () => {
    const blob = new Blob(['\uFEFF', portfolioCsv({ ...currentProfile, rows: holdings, sourceUrl: '', sourceLabel: '' })], { type: 'text/csv;charset=utf-8;' });
  ...
  return <main className="rk-page ip-page">
    ...
    <header className="rk-hero"><div className="rk-container">
      ...
      <div className="rk-hero-meta"><span>{country === 'IN' ? 'INDIA · INR' : 'USA · USD'}</span><span>{currentProfile?.reportPeriod ? `REPORTING PERIOD · ${currentProfile.reportPeriod}` : 'REPORTING PERIOD UNAVAILABLE'}</span></div>
    </div></header>
    <div className="rk-container ip-content">
      ...
      {currentProfile && <>
        <div className="ip-metrics"><div><span>Holdings rows available</span><strong>{holdings.length.toLocaleString()}</strong></div><div><span>Reporting period</span><strong>{currentProfile.reportPeriod || 'Not available'}</strong></div><div><span>Retrieved</span><strong>{currentProfile.retrievedAt}</strong></div></div>
        <aside className="ip-notice"><strong>Daily price-based valuation</strong><p>{valuation?.pricedCount ? `${valuationMoney(valuation.value, country)} · ${valuation.pricedCount} of ${holdings.length} listed rows priced. ${valuation.pricedCount < holdings.length ? 'Partial subtotal; unpriced holdings are excluded.' : 'Value of the disclosed rows only.'}` : 'Price-based valuation is not available yet. Reported values remain visible below.'}</p>{valuation?.pricedCount > 0 && <p>{valuation.dayChangePct != null && `Price movement: ${valuation.dayChangePct > 0 ? '+' : ''}${valuation.dayChangePct.toFixed(2)}% versus prior trading-session close, using the same disclosed quantities. `}Last successful refresh: {valuationTime(valuations.updatedAt)}. {valuationStale(valuations) ? 'Refresh overdue; showing the last successful snapshot. ' : ''}Oldest included quote: {valuationTime(valuation.oldestPriceAt)}.</p>}<p>Scheduled daily at 2:00 AM IST. Yahoo Finance regular-session prices may be delayed; US prices can be intraday at this time. This estimates disclosed holdings, not personal net worth. Share counts remain as reported; unsupported securities and unresolved corporate actions are excluded.</p></aside>
        <aside className="ip-notice"><strong>{currentProfile.coverageLabel}</strong><p>Publicly disclosed positions may cover different reporting dates and may not represent the complete portfolio.</p>{currentProfile.managerName && <p>Filing entity: <strong>{currentProfile.managerName}</strong>. This is the entity’s reported portfolio, not the named individual’s personal assets.</p>}
        </aside>
        {holdings.length > 0 ? <>
          {country === 'IN' && <p className="rk-table-hint">{disclosures.data ? `Holdings checked ${valuationTime(disclosures.data.updatedAt)}. ` : ''}{disclosures.unavailable ? 'Latest refresh unavailable; retained holdings remain visible. ' : ''}Daily checks at 2 AM IST. Dated holder details appear within each company; missing disclosures do not establish a sale.</p>}
          ...
          <p className="rk-table-hint">{['sec13f', 'fund-disclosures'].includes(currentProfile.kind) ? 'Value is at the filing date. Portfolio weight is a share of the disclosed book, not ownership of the company.' : 'Values retain their reported units. Ownership changes are reported percentage-point changes, not investment returns or confirmed trades.'}</p>
          <div className="rk-table-wrap" role="region" aria-label={`${name} holdings`} tabIndex={0}><table><caption className="rk-sr-only">{name} holdings; {currentProfile.reportPeriod}. {currentProfile.coverageLabel}.</caption><thead><tr><th scope="col">Stock / security</th><th scope="col">Holding value {country === 'IN' ? '(INR)' : '(USD)'}</th><th scope="col">Latest price / estimated value</th><th scope="col">Quantity held</th><th scope="col">Reporting date</th><th scope="col">{currentProfile.kind === 'sec13f' ? 'Security type' : currentProfile.kind === 'fund-disclosures' ? 'Reported share change' : 'Reported change (pp)'}</th>{periods.map(period => <th key={period} scope="col">{period}</th>)}</tr></thead><tbody>{shown.map((row, i) => { const priced = row.snapshotIndex == null ? null : valuation?.rows[row.snapshotIndex]; return <tr key={`${page}:${i}`}><th scope="row">{row.stock}{(row.holder || row.security) && <small className="ip-holder">{[row.holder, row.security].filter(Boolean).join(' · ')}</small>}</th><td className="rk-number">{display(row.value)}</td><td className="rk-number">{priced?.value != null ? <><strong>{valuationMoney(priced.value, country)}</strong><small className="ip-holder">{country === 'IN' ? '₹' : '$'}{priced.price.toLocaleString()} · {priced.symbol}</small><small className="ip-holder">{valuationTime(priced.priceAt)}</small></> : <span className="rk-muted">{priced?.reason || 'Not priced'}</span>}</td><td className="rk-number">{display(row.quantity)}</td><td>{row.reportPeriod || currentProfile.reportPeriod}{row.disclosures?.length > 0 && <details><summary>Latest reported holders ({row.disclosures.length})</summary>{row.disclosures.map((d, n) => <p key={n}>{d.holder}{d.bucket && ` · ${{personal: 'Personal direct', family: 'Family / trust', corporate: 'Corporate entity', managed: 'Managed fund assets'}[d.bucket] || 'Direct disclosure'}`} · {d.quantity.toLocaleString('en-IN')} shares · {d.ownershipPct == null ? 'Ownership unavailable' : `${d.ownershipPct}%`} · {d.period}{d.stale ? ' · Previous filing retained' : ''}</p>)}<small>Holder-level disclosures are not added to the portfolio quantity above.</small></details>}</td><td className={disclosureStatus(row) === 'Increased' ? 'rk-positive' : disclosureStatus(row) === 'Reduced' ? 'rk-negative' : ''}>{currentProfile.kind === 'sec13f' ? display(row.security) : display(row.change)}</td>{periods.map((period, index) => <td className="rk-number" key={period}>{display(row.history[index])}</td>)}</tr>; })}</tbody></table>{!shown.length && <div className="rk-no-results"><h3>No matching holdings</h3><button onClick={() => { changeQuery(''); setStatus('all'); }}>Clear filters</button></div>}</div>
