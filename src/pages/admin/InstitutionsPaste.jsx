...
export default function InstitutionsPaste(){
 ...
 async function call(operation){setBusy(true);setError('');setResult(null);try{
  ...
  const response=await fetch(`${API_ORIGIN || ''}/api/intelligence/institutions/${operation}`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${data.session.access_token}`},body:JSON.stringify({text,country,asOf:asOf||null})});
 ...
 return <main className="itp"><header><h1>Institutions — paste & publish</h1><p>Copy the complete table including headers from your source, Excel or Sheets. Check it, then publish to update the website immediately.</p></header>
 <div className="itp-bar"><label>Market <select disabled={busy} value={country} onChange={e=>{setCountry(e.target.value);setText('');reset();}}><option value="IN">India · ₹ crore (Cr)</option><option value="US">USA · $ million (M)</option></select></label><label>Valuation date (optional) <input type="date" disabled={busy} value={asOf} onChange={e=>{setAsOf(e.target.value);reset();}}/></label></div>
 <div className="itp-help"><p>Columns: Superstar · Portfolio Value* (change) · #Of Stocks · Sector Preference · Quarterly Net Worth · Top Holdings · Recently bought · Recently sold.</p><p>Both screenshot formats are supported. Portfolio value examples: India “334,805.84 Cr ↓ -5.62%”; USA “313,025.25 M ↑ 2.29%”. Tab-separated, CSV and Markdown tables work. Copy the actual table, not a screenshot. Graph images do not contain transferable quarterly figures; missing history stays blank.</p><p><strong>Publish replaces the entire {country==='IN'?'India':'USA'} table.</strong> Paste all investors you want shown for this market. The other market is unchanged.</p></div>
 {error&&<div className="itp-error" role="alert" style={{whiteSpace:'pre-wrap'}}>{error}</div>}
 {result&&<div className="itp-ok" role="status">Published {result.row_count} investors. <Link to={`/institutions?country=${country}`} target="_blank">View the updated page ↗</Link></div>}
