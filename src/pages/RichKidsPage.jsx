...
export default function InstitutionsPage() {
 ...
 const [selections,setSelections] = useState(()=>{try{const saved=JSON.parse(localStorage.getItem('agi-investor-selections')||'{}');return Object.fromEntries(['IN','US'].map(code=>[code,Array.isArray(saved?.[code])?saved[code].filter(x=>typeof x==='string'):[]]));}catch{return {IN:[],US:[]};}});
 ...
 const selected=selections[country]||[];
 const changeSelection=names=>setSelections(previous=>({...previous,[country]:names}));
  ...
  <div className="rk-container">
   ...
   <section id="rk-panel" role="tabpanel" aria-labelledby={`rk-tab-${country}`} tabIndex={0}>
    ...
    {error?<div className="rk-empty" role="alert">{error}</div>:!current?<p role="status">Loading published portfolios…</p>:investors.length ? <>
     <InvestorSelector key={country} investors={investors} selected={selected} onChange={changeSelection} />
     <PortfolioCharts investors={rows} country={country} />
