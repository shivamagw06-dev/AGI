 ...
 return <main className="rk-page">
  ...
  <div className="rk-container">
   ...
   <section id="rk-panel" role="tabpanel" aria-labelledby={`rk-tab-${country}`} tabIndex={0}>
    ...
    <p className="rk-table-hint">Daily valuations: {valuations ? `last successful refresh ${valuationTime(valuations.updatedAt)}${valuationStale(valuations) ? ' · refresh overdue' : ''}` : 'awaiting first successful price refresh'}. Scheduled at 2:00 AM IST. Estimates cover priced disclosed shares, not personal net worth.</p>
    ...
    {error?<div className="rk-empty" role="alert">{error}</div>:!current?<p role="status">Loading published portfolios…</p>:investors.length ? <>
     ...
     <div className="rk-table-wrap" role="region" aria-label="Investor portfolio comparison" tabIndex={0}><table><caption className="rk-sr-only">{country==='IN'?'Indian':'US'} investor portfolios. Currency: {country==='IN'?'INR crore':'USD million'}.</caption><thead><tr>{['Superstar','Portfolio Value* (change)','Daily estimated holdings value','# of Stocks','Sector Preference','Quarterly Net Worth','Top Holdings','Recently bought','Recently sold'].map(label=><th key={label} scope="col">{label}</th>)}</tr></thead><tbody>{rows.map(row=>{const valuation=valuations?.profiles?.[`${country.toLowerCase()}-${investorSlug(row.name)}`]; return <tr key={row.name}>
      <th scope="row"><Link to={investorPath(country,row.name)}>{row.name}<span className="rk-external" aria-hidden="true"> →</span></Link></th>
      <td className="rk-number"><strong>{currency}{row.value.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})} {unit}</strong><span className={row.change>=0?'rk-positive':'rk-negative'}>{row.change==null?'Not supplied':`${row.change>0?'+':''}${row.change}%`}</span></td><td className="rk-number">{valuation?.pricedCount ? <><strong>{valuationMoney(valuation.value,country)}</strong><span>{valuation.pricedCount}/{valuation.rowCount} rows priced{valuation.pricedCount < valuation.rowCount ? ' · partial subtotal' : ''}</span><span>Disclosed: {valuation.reportPeriod}</span>{valuation.dayChangePct != null && <span className={valuation.dayChangePct >= 0 ? 'rk-positive' : 'rk-negative'}>{valuation.dayChangePct > 0 ? '+' : ''}{valuation.dayChangePct.toFixed(2)}% price movement</span>}</> : <span className="rk-muted">Not priced</span>}</td><td className="rk-count">{row.stocks}</td><td><Entries items={row.sectors}/></td><td><span className="rk-muted">{row.quarterlyNetWorth || 'Not supplied'}</span></td><td><Entries items={row.holdings}/></td><td><Entries items={row.bought} tone="rk-positive"/></td><td><Entries items={row.sold} tone="rk-negative"/></td>
