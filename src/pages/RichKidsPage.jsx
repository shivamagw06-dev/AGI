 ...
 return <main className="rk-page institutions-directory">
  ...
  <div className="rk-container">
   ...
   <section id="rk-panel" role="tabpanel" aria-labelledby={`rk-tab-${country}`} tabIndex={0}>
    <div className="institutions-status"><p><span className={`institutions-status-dot ${valuationStale(valuations) ? 'is-stale' : ''}`} aria-hidden="true"/>{category==='institutional'?'Institutional snapshot · daily pricing not connected':valuations ? `Prices refreshed ${valuationTime(valuations.updatedAt)}${valuationStale(valuations) ? ' · refresh overdue' : ''}` : 'Price refresh pending'}{category==='individual'&&<span className="institutions-schedule">Daily refresh · 2:00 AM IST</span>}</p><button className="rk-refresh" onClick={()=>setRefresh(x=>x+1)}>↻ Refresh table</button></div>
    {error?<div className="rk-empty" role="alert">{error}</div>:!current?<p role="status">Loading published portfolios…</p>:investors.length ? <>
     {category==='institutional'&&!current.published&&<p className="rk-table-hint">Starting coverage: three institutions transcribed from the supplied screenshot. This is not a complete institutional universe; valuation date was not supplied.</p>}
     <InvestorSelector key={`${country}-${category}`} investors={investors} selected={selected} onChange={changeSelection} />
     <PortfolioCharts investors={rows} country={country} linkProfiles={category==='individual'} />
     ...
     <p className="rk-table-hint">{category==='institutional'?'Institutional snapshots are updated when published by the administrator. Detailed holdings and daily pricing are not yet connected for this category.':'Select an investor to view holdings. Daily estimates exclude unpriced positions; partial totals are marked.'}</p>
     <div className="rk-table-wrap" role="region" aria-label="Investor portfolio comparison" tabIndex={0}><table><caption className="rk-sr-only">{country==='IN'?'Indian':'US'} investor portfolios. Currency: {country==='IN'?'INR crore':'USD million'}.</caption><thead><tr>{['Investor','Reported value','Daily estimate','Stocks','Sector exposure',...(hasQuarterlyHistory ? ['Quarterly history'] : []),'Top holdings','Reported additions','Reported reductions'].map(label=><th key={label} scope="col">{label}</th>)}</tr></thead><tbody>{rows.map(row=>{const valuation=category==='individual'&&valuations?.profiles?.[`${country.toLowerCase()}-${investorSlug(row.name)}`]; return <tr key={row.name}>
      <th scope="row">{category==='individual'?<Link to={investorPath(country,row.name)}>{row.name}<span className="rk-external" aria-hidden="true"> →</span></Link>:row.name}</th>
