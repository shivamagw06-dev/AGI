...
/**
 * That this manager wrote something, and where to read it.
 *
 * The sentences themselves are on the manager's own page. Fifty managers are
 * listed here and six have a document, so the full block repeated one card's
 * worth of prose down a page whose job is to help someone choose which manager
 * to open.
 */
function SaidLink({ manager }) {
  const said = manager.said;
  if (!said?.approved_count) return null;
  const [document] = said.publications || [];
  return (
    <Link
      to={`/institutional-holdings/funds/${manager.slug}`}
      className="mt-4 flex items-baseline justify-between gap-3 border-t border-[#f0f0f0] pt-3 text-[11px] hover:text-[#ff8000]"
    >
      <span className="min-w-0 truncate font-semibold text-[#444444]">
        In its own words
        {document ? <span className="font-normal text-[#8a8a8a]">{` · ${document.title}`}</span> : null}
      </span>
      <span className="shrink-0 tabular-nums text-[#767676]">{said.approved_count}</span>
    </Link>
  );
}
...
function ManagerProfile({ manager }) {
  ...
  return (
    <article className="rounded-2xl border border-[#e6e6e6] bg-white p-5 transition hover:border-[#cfcfcf]">
      ...
      {/* Outside the strategy branch on purpose: a manager can have a pasted
          document and no strategy profile, and the document is the part that
          is in its own words. */}
      <SaidLink manager={manager} />
    </article>
  );
}
